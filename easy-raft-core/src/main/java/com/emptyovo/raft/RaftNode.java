...
import com.emptyovo.raft.storage.SegmentedLog;
import com.emptyovo.raft.storage.Snapshot;
import com.emptyovo.raft.util.ConfigurationUtils;
...
/**
 * 1. Save raft node core data (node status information, log information, snapshot, etc.)
...
public class RaftNode {
    ...
    // id of the candidate who is currently receiving votes
    private int votedFor;
    private int leaderId; // leader节点id
    // index value of the largest known log entry that has been submitted
    ...
    public boolean replicate(byte[] data, RaftProto.EntryType entryType) {
        ...
        try {
            ...
            if (raftOptions.isAsyncWrite()) {
                // After the primary node is successfully written, returns.
                return true;
            }
    ...
    /**
     * The client initiates a pre-vote request
     * pre-vote/vote is a typical two-phase implementation.
     * aims to prevent a node from continuously increasing the number of terms to initiate voting
     * when the network is disconnected.
     * After the network of this node recovers, the term of other nodes in the cluster increases,
     * and the cluster status changes.
     */
    ...
    /**
     * The client initiates a formal vote, which is valid for the candidate
     */
    ...
    /**
     * The client initiates a formal vote request
     * @param peer Server node info
     */
